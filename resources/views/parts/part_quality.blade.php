...
@section('content')
    ...
    <div class="main-content">
        <div class="page-content">
            ...
            <p class="fs-17 text-bg-dark text-center">الكفاءة </p>
         
                <div class="card">
                
                    <div class="card-body">
                        <div class="col-12 px-1 m-3 text-end">
                            <button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#staticBackdrop">إضافة
                                </button>
                        </div>
                        <table id="itemtbl"  class="table table-striped table-bordered cell-border  mt-3" style="font-size: smaller;width:100%">
                            <thead style="background: #67b1736e;"> 
                                <tr>
                                    <th class="d-none">part quality_id</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">  تعديل الكفاءة </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('part_quality.update', 'test') }}" method="post" enctype="multipart/form-data">
                    {{ method_field('patch') }}
                    {{ csrf_field() }}
                            ...
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <div class="row p-3">
                                    <div class="col-12">
                                        ...
                                        <input name="part_quality_id" id="part_quality_id" type="hidden" class="form-control">
                                        <input name="name" id="part_quality_name" type="text" class="form-control"
                                            required="">
                                    </div>
                                </div>
                                <div class="row">
                                    ...
                                    <div class="col-12">
                                        <textarea name="note" cols="30" rows="5" id="part_quality_note" class="form-control"></textarea>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="staticBackdrop_delete" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">  حذف الكفاءة</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('part_quality.destroy', 'test') }}" method="post">
                    {{ method_field('Delete') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <div class="row p-3">
                                    <div class="col-12">
                                        <input id="part_quality_id_delete" name="part quality_id" type="hidden" class=""
                                            required="">
                                        <label for="recipient-name" class="col-form-label"> سيتم حذف هذه الوحدة </label>

                                        <input id="part_quality_name_delete" name="name" type="text"
    ...
    <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">   إضافة كفاءة جديدة  </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('part_quality.store') }}" method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="modal-body">
...
@section('js')
   <script>
        ...
        $(document).ready(function() {
            ...
            itemtbl.clear();
            itemtbl.rows.add(all_parts).draw();
    ...
    <script>
        $('#itemtbl tbody').on('click', '.edit_item', function() {
            $("#part_quality_id").val('');
            $("#part_quality_name").val('');
            $("#part_quality_note").val('');

            var currentRow = $(this).closest("tr");
            $("#part_quality_id").val(currentRow.find("td:eq(0)").text());
            $("#part_quality_name").val(currentRow.find("td:eq(1)").text());
            $("#part_quality_note").val(currentRow.find("td:eq(2)").text());
        ...
        $('#itemtbl tbody').on('click', '.delete_item', function() {
            $("#part_quality_id_delete").val('');
            $("#part_quality_name_delete").val('');
            var currentRow = $(this).closest("tr");
            $("#part_quality_id_delete").val(currentRow.find("td:eq(0)").text());
            $("#part_quality_name_delete").val(currentRow.find("td:eq(1)").text());
