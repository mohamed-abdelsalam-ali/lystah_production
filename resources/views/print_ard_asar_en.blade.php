...
<table id="partsDT" class="table table-striped table-bordered cell-border " style="width:100%;direction: ltr">
    ...
    <tbody class="tm_text-center" id="partsDT_body">
        @foreach ( $presaleOrders->presaleorderpart as $item )
            <tr>
                ...
                @if ($item->type == 2)
                    <td>
                        <span>{{  explode("/", $item->part->name_eng)[0] }}</span><br>
                ...
                @else
                    <td >{{ ($item->part->name_eng) ? $item->part->name_eng : 'No Eng Name' }}</td>
                ...
                @if ($showflag)
                <td >
                {{ (count($item->oem_number) > 0) ? $item->oem_number[0]->number : 'No OEM' }}
            </td>
                @else

                @endif
                <td >{{ $item->source->name_en }}</td>
...
<table class="table table-striped table-bordered cell-border tm_text_center" style="width:100%">
    <thead>
        <tr>
            <th> Sub Total</th>
            <th>Tax</th>
...
@endif
<p style="width:100%">
    Note : This Offer Is Valid Until Date <strong> {{explode(' ',$presaleOrders->due_date)[0]}}</strong>
</p>
