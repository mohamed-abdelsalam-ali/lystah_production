...
@section('content')
    ...
    <div class="main-content">
        <div class="page-content">
            <h1 class="text-center text-info">series
            </h1>
            <div class="row">
                <div class="col">
                    <div class="card">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Edit series</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('series.update', 'test') }}" method="post" enctype="multipart/form-data">
                    {{ method_field('patch') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <div class="row p-3">
                                    <div class="col-12">
                                        <input name="series_id" id="series_id" type="hidden" class="form-control" required="">

                                        <label for="recipient-name" class="col-form-label"> series Name </label>
                                        <input name="name" id="series_name" type="text" class="form-control" required="">
                                    </div>
                                ...
                                <div class="row p-3">
                                    <div class="col-12">
                                        ...
                                        <select name="model_id" id="model_id_edit"
                                            class="selectpicker button btn btn-light form-control">
                                            @foreach ($models as $model)
                                                <option class="text-center" value="{{ $model->id }}">
                                                    {{ $model->name }}</option>
                                            @endforeach
                                ...
                                <input type="file" class="form-control" name="seris_img" id="series_image_edit">
                                <div class="row">
                                    <label for="recipient-name" class="col-form-label"> Description </label>
                                    <div class="col-12">
                                        <textarea name="desc" id="series_desc" cols="30" rows="5" class="form-control"></textarea>
                                    </div>
                                </div>
    ...
    <div class="modal fade" id="staticBackdrop_delete" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('series.destroy', 'test') }}" method="post">
                    {{ method_field('Delete') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <div class="row p-3">
                                    <div class="col-12">
                                        <input id="series_id_delete" name="series_id" type="hidden" class=""
                                            required="">
                                        <label for="recipient-name" class="col-form-label"> سيتم حذف هذا الصنف </label>

                                        <input id="series_name_delete" name="name" type="text" class="form-control"
                                            readonly>
                                    </div>
    ...
    <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel"> Add New series </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                            ...
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <div class="row p-3">
                                    <div class="col-12">
                                        ...
                                        <label for="recipient-name" class="col-form-label"> Model Name </label>
                                        <select name="model_id" id="model_id_add"
                                            class="selectpicker button btn btn-light form-control">
                                            @foreach ($models as $model)
                                                <option class="text-center" value="{{ $model->id }}">
                                                    {{ $model->name }}</option>
                                ...
                                <div class="row pt-3 d-none" id="div_image_series">
                                    <img style="width: 200px; height: 100px;" class="add_logo_image" src=""
                                        class="rounded border" alt="">
                                </div>
                                <input type="file" class="form-control" id="add_logo" name="seris_img">
                                <div class="row">
                                    <label for="recipient-name" class="col-form-label"> Description </label>
                                    <div class="col-12">
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        "render": function(data) {
                            if (data) {
                                return '<img  class="avatar-img img-thumbnail rounded-5 itemimg" src="series_images/' +
                                    data +
                                    '" width="40px">';
            ...
            itemtbl.clear();
            itemtbl.rows.add(all_series).draw();
    ...
    <script>
        $('#itemtbl tbody').on('click', '.edit_item', function() {
            ...
            $("#series_id").val(currentRow.find("td:eq(0)").text());
            $("#series_name").val(currentRow.find("td:eq(1)").text());
            $("#model_id_edit").val(currentRow.find("td:eq(2)").text());
            if (currentRow.find("td:eq(8)").children().attr('src')) {
                $("#div_image_edit").removeClass('d-none');
                $('.displayImagee').attr('src', currentRow.find("td:eq(8)").children().attr('src'));
            } else {
                $("#div_image_edit").addClass('d-none');
                $('.displayImagee').attr('src', '');

            }
            $("#series_desc").val(currentRow.find("td:eq(9)").text());
        ...
        $('#itemtbl tbody').on('click', '.delete_item', function() {
            $("#series_id_delete").val('');
            $("#series_name_delete").val('');
            var currentRow = $(this).closest("tr");
            $("#series_id_delete").val(currentRow.find("td:eq(0)").text());
            $("#series_name_delete").val(currentRow.find("td:eq(1)").text());
    ...
    <script>
     $('#series_image_edit').on('change', function() {
            var input = this;
            var url = $(this).val();
        ...
        $('#add_logo').on('change', function() {
            ...
            if (input.files && input.files[0] && (ext == "gif" || ext == "png" || ext == "jpeg" || ext == "jpg")) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    $("#div_image_series").removeClass('d-none');
