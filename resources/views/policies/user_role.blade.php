...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            drawPartTbl(users, roles, user_roles);


        });

        function drawPartTbl(users, roles, user_roles) {
            var arr = {};
            var JSONResult = [];
            arr[0] = "مستخدمين/وظائف"
            ...
            for (let i = 0; i < users.length; i++) {
                ...
                row_dtable.push(users[i].username);
                for (let j = 1; j < Object.keys(JSONResult[0]).length + 1; j++) {
                    ...
                    for (let k = 0; k < user_roles.length; k++) {
                        ...
                        // console.log("------------------------------")
                        if (Object.keys(JSONResult[0])[j] == user_roles[k].role_id && users[i].id == user_roles[k]
                            .model_id) {
                            checker = 1;
                        } else {
               ...
               <input class="roles" type="checkbox" checked  data-role="${Object.keys(JSONResult[0])[j]}" data-user="${users[i].id}">
               <span class="slider round"></span>
             </label>`);
                    } else {
                        row_dtable.push(`<label class="switch"  >
               <input class="roles"  type="checkbox" data-role="${Object.keys(JSONResult[0])[j]}" data-user="${users[i].id}">
               <span class="slider round"></span>
             </label>`);
                    }
