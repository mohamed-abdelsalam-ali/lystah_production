...
@section('content')
    ...
    <div class="main-content">
        <div class="page-content">
            <h1 class="text-center text-info"> {{$sub_group->name}} Catalogs For
            </h1>
            <div class="row">
                <div class="col">
                    <div class="card">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Edit catalog Image</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('catalog.update', 'test') }}" method="post" enctype="multipart/form-data">
                    {{ method_field('patch') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <input type="file" class="form-control" name="image_url" id="catalog_image_edit">
                                <input type="hidden" class="form-control" id="category_id_edit" name="category_id">


                                <input type="hidden" class="form-control" value="{{$sub_group->id}}" name="sub_group_id">

                                <div class="row">
                                    <label for="recipient-name" class="col-form-label"> Description </label>
                                    <div class="col-12">
                                        <textarea name="desc" cols="30" rows="5" id="catalog_desc" class="form-control"></textarea>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="staticBackdrop_delete" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Delete catalog Image</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('catalog.destroy', 'test') }}" method="post">
                    {{ method_field('Delete') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <div class="row p-3">
                                    <div class="col-12">
                                        <input id="catalog_id_delete" name="catalog_id" type="hidden" class=""
                                            required="">
                                        <label for="recipient-name" class="col-form-label"> سيتم حذف هذه الصورة </label>

                                        <div class="row pt-3  d-none" id="div_image_delete">

                                            <img style="width: 200px; height: 100px;" class="displayImagee_delete" src=""
                                                class="rounded border" alt="">
                                        </div>
                                    </div>
    ...
    <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel"> Add New catalog Image </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('catalog.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                aria-labelledby="home-tab">
                                <div class="row pt-3 d-none" id="div_image_catalog">
                                    <img style="width: 200px; height: 100px;" class="add_logo_image" src=""
                                        class="rounded border" alt="">
                                </div>
                                <input type="file" class="form-control" id="add_logo" name="image_url">
                                <input type="hidden" class="form-control" value="{{$sub_group->id}}" name="sub_group_id">
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        "render": function(data) {
                            if (data) {
                                return '<img  class="avatar-img img-thumbnail rounded-5 itemimg" src="/catalog_images/' +
                                    data +
                                    '" width="40px">';
            ...
            itemtbl.clear();
            itemtbl.rows.add(all_catalogs).draw();
    ...
    <script>
        $('#itemtbl tbody').on('click', '.edit_item', function() {
            ...
            $("#category_id_edit").val(currentRow.find("td:eq(0)").text());
            if (currentRow.find("td:eq(1)").children().attr('src')) {
                $("#div_image_edit").removeClass('d-none');
                $('.displayImagee').attr('src', currentRow.find("td:eq(1)").children().attr('src'));
            } else {
                $("#div_image_edit").addClass('d-none');
                $('.displayImagee').attr('src', '');

            }
            $("#catalog_desc").val(currentRow.find("td:eq(2)").text());
        ...
        $('#itemtbl tbody').on('click', '.delete_item', function() {
            $("#catalog_id_delete").val('');
            $('.displayImagee_delete').attr('src','');
            var currentRow = $(this).closest("tr");
            $("#catalog_id_delete").val(currentRow.find("td:eq(0)").text());
            if (currentRow.find("td:eq(1)").children().attr('src')) {
                $("#div_image_delete").removeClass('d-none');
                $('.displayImagee_delete').attr('src', currentRow.find("td:eq(1)").children().attr('src'));
            } else {
                $("#div_image_delete").addClass('d-none');
                $('.displayImagee_delete').attr('src', '');
    ...
    <script>
        $('#catalog_image_edit').on('change', function() {
            var input = this;
            var url = $(this).val();
        ...
        $('#add_logo').on('change', function() {
            ...
            if (input.files && input.files[0] && (ext == "gif" || ext == "png" || ext == "jpeg" || ext == "jpg")) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    $("#div_image_catalog").removeClass('d-none');
