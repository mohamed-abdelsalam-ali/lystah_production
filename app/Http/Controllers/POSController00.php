...
class POSController extends Controller
{
     ...
     public function shop(Request $request)
    {
        ...
        // return $allItems->flatten();
        // return  $allItems;

        // $query1 = QueryBuilder::for(resolve($entity_tbl)->newQuery())
        //     ->join('stores_log', $store->table_name . '.store_log_id', '=', 'stores_log.id')
        //     ->join('all_parts', 'stores_log.All_part_id', '=', 'all_parts.id')
        //     ->join('sale_pricing', function ($join) {
        //         $join->on('all_parts.part_id', '=', 'sale_pricing.part_id')
        //             ->on('all_parts.source_id', '=', 'sale_pricing.source_id')
        //             ->on('all_parts.status_id', '=', 'sale_pricing.status_id')
        //             ->on('all_parts.quality_id', '=', 'sale_pricing.quality_id')
        //             ->where('sale_pricing.type_id', 1)
        //             ->whereNull('sale_pricing.to');
        //     }) // Join the max_pricing relationship table
        ...
        //         'stores_log.all_parts.max_pricing.sale_type',
        ...
        //     ])
        //     ->allowedFilters([
        //         // 'stores_log.all_parts.part.part_details.id',
        //         AllowedFilter::scope('part_name'),
        //         AllowedFilter::scope('part_brand'),
        //         AllowedFilter::scope('part_group'),
        //         AllowedFilter::scope('part_supplier'),
        //         AllowedFilter::scope('part_subgroup'),
        ...
        //         // AllowedFilter::scope('part_type'),
        //         AllowedFilter::scope('part_model'),
        //         AllowedFilter::scope('part_series'),
        //         // AllowedFilter::scope('part_type'),


        //         AllowedFilter::scope('part_max_price'),
        //         AllowedFilter::scope('part_min_price'),
        //     ]);

        // $count_items = 0;
        // $query2 = QueryBuilder::for(resolve($entity_tbl)->newQuery())
        //     ->join('stores_log', $store->table_name . '.store_log_id', '=', 'stores_log.id')
        ...
        //         // AllowedFilter::scope('part_min_price'),
        //     ]);
        // $query3 = QueryBuilder::for(resolve($entity_tbl)->newQuery())
        // ->join('stores_log', $store->table_name . '.store_log_id', '=', 'stores_log.id')
        ...
        //     // AllowedFilter::scope('part_min_price'),
        // ]);
        // return $allItems = $query1->union($query2)->paginate(9);


        // return   $allItems1 = $query1->paginate(50)->shuffle();
        // $allItems2 = $query2->paginate(9);
        //  $mergedItems = $allItems1->concat($allItems2)->shuffle()->paginate(9);
        // return  $mergedItems;
        ...
        // Merge the items from both paginations

        // $count_items = $query->get()->count();
        // return $count_items;

    }

    public function printposOLD1609(Request $request)
    {
        // return $request;
        ...
        try {
            ...
            DB::rollback();
            session()->flash("success", "لا يمكن حفظ الفاتورة");
            ...
            for ($i = 0; $i < count($inv_items); $i++) {
                ...
                //// remove from store ///////////////////
                 if ($type_id == 1) {
                    ...
                    $allparts = AllPart::where('remain_amount', '>', 0)->where('part_id', $part_id)->where('source_id', $source_id)
                    ->where('status_id', $status_id)->where('quality_id', $quality_id)->orderBy('id', 'ASC')->with('order_supplier')->get();
                    $all_part_table = 'all_parts';
                        ...
                        if ($element->remain_amount >= $requestAmount0) {
                            ...
                            $store_table_name = $store[0]->table_name;
                             ...
                             $returnRes=DB::table($store_table_name)
                                ...
                                ->join('stores_log', $store_table_name.'.store_log_id', '=', 'stores_log.id')
                                ->join($all_part_table, function($join) use ($store_table_name, $all_part_table,$store_id) {
                                    $join->on($store_table_name.'.supplier_order_id', '=', $all_part_table.'.order_supplier_id')
                                        ->on('stores_log.All_part_id', '=', $all_part_table.'.id');
                                })
                    ...
                    // $allpp = AllPart::where('remain_amount', '>', 0)->where('part_id', $part_id)->where('source_id', $source_id)
                    // ->where('status_id', $status_id)->where('quality_id', $quality_id)->orderBy('id', 'ASC')->get();
                    // $requestAmount = $amount;
                    // foreach ($allpp as $key => $element) {
   ...
   public function saveMadyonea(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return view('sanad.print_sanad_abd', compact('person_id','payment_types', 'paperTitle', 'recordName', 'recordValue', 'recoredUrl', 'moneyVal', 'personName', 'datee'));
    ...
    public function saveMadyoneasup(Request $request)
    {
        ...
        try {
                ...
                $datee = $mad->date;
                DB::commit();
                return view('sanad.print_sanad_sarf', compact('payment_types', 'paperTitle', 'recordName', 'recordValue', 'recoredUrl', 'moneyVal', 'personName', 'datee'));
   ...
   public function refund($invId,$storeId)
    ...
    public function saveRefundOLD1609(Request $request)
    {
        // return $request;
        ...
        try {
            ...
            foreach ($request->refunditem as $key => $value) {
                ...
                $refundAmount = $request->refundAmount[$key];
                ...
                if ($refundAmount > 0) {
                    ...
                    if ($request->part_type_id[$key] == '1') {
                        $allpart = AllPart::where('part_id', $partId)->where('source_id', $sourceid)->where('status_id', $statusid)->where('quality_id', $qualityid)->orderBy('id', 'Desc')->get();
                        ...
                        foreach ($allpart as $keyx => $valuex) {
                            if ($valuex->remain_amount + $totalAmount  <= $valuex->amount) {
                                ...
                                AllPart::where('id', $valuex->id)->increment('remain_amount', $valuex->amount - $valuex->remain_amount);
                                RefundInvoice::create([
                                    'invoice_id' => $invoiceId,
                                    'item_id' => $valuex->id,
                                ...
                                array_push($orderSupplierArr, $valuex->order_supplier_id);
                                $totalAmount = $totalAmount - ($valuex->amount - $valuex->remain_amount);
                        ...
                        $allpart = AllWheel::where('part_id', $partId)->where('source_id', $sourceid)->where('status_id', $statusid)->where('quality_id', $qualityid)->where('remain_amount', '>', 0)->orderBy('id', 'Desc')->get();
                        ...
                        $allpart = AllKit::where('part_id', $partId)->where('source_id', $sourceid)->where('status_id', $statusid)->where('quality_id', $qualityid)->where('remain_amount', '>', 0)->orderBy('id', 'Desc')->get();
            ...
            if (floatVal($request->totalRef) > 0) {
                if ($request->paymnet == 0) {
                    ...
                    array_push($quaditems, (object) ['acountant_id' => $request->paymentAcount, 'madin' => $request->refundTotal1, 'dayin' => 0]); // طريقة الدفع
                    array_push($quaditems, (object) ['acountant_id' => 4512, 'madin' => 0, 'dayin' => $request->totalRef]); //مردودات مبيعات
                    array_push($quaditems, (object) ['acountant_id' => $clientHessap->accountant_number, 'madin' =>  floatval($request->totalRef) - floatval($request->refundTotal1), 'dayin' => 0]); // زبون
            ...
            $date = Carbon::now();
            $type = null;
            $notes = 'مردودات مبيعات ' . $store[0]->name;
            $automaicQayd->AutomaticQayd($quaditems, $date, $type, $notes);
                    ...
                    if ($request->part_type_id[$key] == '1') {
                        ...
                        foreach ($allpart as $keyx => $valuex) {
                            if ($valuex->remain_amount + $totalAmount  <= $valuex->amount) {
                                ...
                                // dd($valuex->amount - $valuex->remain_amount);
                                RefundInvoice::create([
                                    'invoice_id' => $invoiceId,
                                    'item_id' => $valuex->id,
                                    'r_amount' => $valuex->amount - $valuex->remain_amount,
    ...
    //         /*********************************************/
    //         // return $entity_tbl;
    //          $query = $entity_tbl::with([
    //             'stores_log.all_parts.part.part_numbers',
    //             'stores_log.all_parts.source',
    //             'stores_log.all_parts.status',
    //             'stores_log.all_parts.part_quality',
    //             'stores_log.all_parts.sections.store',
    //             'stores_log.all_parts.sections.store_structure',
    ...
    //             // Query for type_id 6
    //             $query3 = $entity_tbl::with([
    //                 'stores_log.all_wheels.source',
    //                 'stores_log.all_wheels.status',
    //                 'stores_log.all_wheels.part_quality',
    //                 'stores_log.all_wheels.sections.store',
    //                 'stores_log.all_wheels.sections.store_structure',
    //                 'stores_log.all_wheels.pricing.sale_type',
    //                 'stores_log.all_wheels.order_supplier.buy_transaction',
    //                 'stores_log.all_wheels.wheel.sub_group.group',
    //                 'stores_log.all_wheels.wheel.wheel_details.wheel_spec',
    //                 'stores_log.all_wheels.wheel.wheel_model.series',
    //                 'stores_log.all_wheels.wheel.wheel_images',
    //             ])->where('type_id', 2)->get();
    //             // Merge all queries
    ...
    //             if($row->type_id == 1){
    //                 if(count($row->stores_log->all_parts) > 0){
    //                     if(count($row->stores_log->all_parts[0]->sections) > 0){
    //                         for ($i=0; $i < count($row->stores_log->all_parts[0]->sections) ; $i++) {
    //                             $btn .='<li>'.$row->stores_log->all_parts[0]->sections[$i]->store_structure->name.'</li>';
    //                         }
    //                     }
    //                 }else{
    //                         $btn="No Section";
    ...
    //             }elseif($row->type_id == 6){
    //                 if(count($row->stores_log->all_kits) > 0){
    //                     if(isset($row->stores_log->all_kits[0]->sections)){
    //                         if(count($row->stores_log->all_kits[0]->sections) > 0){
    //                                 for ($i=0; $i < count($row->stores_log->all_kits[0]->sections) ; $i++) {
    //                                         $btn .='<li>'.$row->stores_log->all_kits[0]->sections[$i]->store_structure->name.'</li>';
    //                                 }
    //                             }
    //                     }else{
    //                         $btn="No Section";
    ...
    //             } elseif($row->type_id == 2){
    //                 if(count($row->stores_log->all_wheels) > 0){
    //                     if(count($row->stores_log->all_wheels[0]->sections) > 0){
    //                         for ($i=0; $i < count($row->stores_log->all_wheels[0]->sections) ; $i++) {
    //                             $btn .='<li>'.$row->stores_log->all_wheels[0]->sections[$i]->store_structure->name.'</li>';
    //                         }
    //                     }
    //                 }else{
    //                         $btn="No Section";
    ...
    //                                     onclick=\'addtoInvoice(this, ' . $part_specs_json . ', ' . json_encode($row->part_id) . ', ' . $name . ', ' . json_encode($row->stores_log->all_parts[0]->source_id) . ', ' . json_encode($row->stores_log->all_parts[0]->status_id) . ', ' . json_encode($row->stores_log->all_parts[0]->quality_id) . ', ' . $SaleType . ', ' . $price . ', ' . $Tamount . ', ' . $type_id . ', ' . $prices_json . ')\'>
    ...
    //                                     aria-describedby="basic-addon1" style="width: 50% !important;">
    //                                 <button class="btn fs-2  text-secondary" style="background-color:#a8e4f2"
    //                                     onclick=\'addtoInvoice(this, ' . $part_specs_json . ', ' . json_encode($row->part_id) . ', ' . $name . ', ' . json_encode($row->stores_log->all_kits[0]->source_id) . ', ' . json_encode($row->stores_log->all_kits[0]->status_id) . ', ' . json_encode($row->stores_log->all_kits[0]->quality_id) . ', ' . $SaleType . ', ' . $price . ', ' . $Tamount . ', ' . $type_id . ', ' . $prices_json . ')\'>
    //                                     <i class="fs-22 mdi mdi-cart-variant"></i>
    //                                 </button>
    //                             </div>';
    //                     return $btn;
    ...
    //                                     aria-describedby="basic-addon1" style="width: 50% !important;">
    //                                 <button class="btn fs-2  text-secondary" style="background-color:#a8e4f2"
    //                                     onclick=\'addtoInvoice(this, ' . $part_specs_json . ', ' . json_encode($row->part_id) . ', ' . $name . ', ' . json_encode($row->stores_log->all_wheels[0]->source_id) . ', ' . json_encode($row->stores_log->all_wheels[0]->status_id) . ', ' . json_encode($row->stores_log->all_wheels[0]->quality_id) . ', ' . $SaleType . ', ' . $price . ', ' . $Tamount . ', ' . $type_id . ', ' . $prices_json . ')\'>
    //                                     <i class="fs-22 mdi mdi-cart-variant"></i>
    //                                 </button>
    //                             </div>';
    //                     return $btn;
    ...
    //             return '';
    //         })
    //         ->rawColumns(['view','name','source','status','quality','amount','section','price','partnumbers','action'])
    //         ->setTotalRecords(50)
    ...
    public function newPos(Request $request)
    {
        ...
        // return $store_safe;
        // return view('testpos', compact('bank_types', 'store_safe', 'allBrands', 'store_data', 'alltaxes', 'clients', 'allSections', 'allClients', 'store_inbox', 'allStores', 'allGroups', 'allSGroups', 'Btype', 'allbrand', 'allmodel', 'allseries', 'allprices'));
        return view('testpos', compact('allStores', 'store_inbox', 'allSections', 'allClients', 'store_data', 'clients', 'allprices', 'alltaxes', 'bank_types', 'store_safe', 'allGroups', 'allSGroups', 'Btype', 'allbrand', 'allmodel', 'allseries'));
    }
    ...
    public function posSearch(Request $request)
    {
        ...
        return view('posSearch', compact('allStores', 'store_inbox', 'allSections', 'allClients', 'store_data', 'clients', 'allprices', 'alltaxes', 'bank_types', 'store_safe', 'allGroups', 'allSGroups', 'Btype', 'allbrand', 'allmodel', 'allseries'));
    }
    public function allDataForId(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return FacadesDataTables::of($query)
                ...
                ->rawColumns(['view', 'name', 'source', 'status', 'quality', 'amount', 'section', 'price', 'partnumbers', 'action', 'send','talef'])
                ->setTotalRecords(20)
                // ->filter(function ($query) {
                //     return $query;
                // })
    ...
    //         $query->whereRaw('CONCAT("FN1", part_id, source_id, status_id, quality_id) = ?', [$searchkey]);
    ...
    //         $query->whereRaw('CONCAT("FN6", part_id, source_id, status_id, quality_id) = ?', [$searchkey]);
    ...
    //         $query->whereRaw('CONCAT("FN2", part_id, source_id, status_id, quality_id) = ?', [$searchkey]);
    //     })
    ...
    //     $tractors = Tractor::where('name','LIKE','%'.$searchkey.'%')->orwhere('tractor_number','LIKE','%'.$searchkey.'%')->orwhere('motornumber','LIKE','%'.$searchkey.'%')->addSelect('*',DB::raw("'Tractor' as type"),DB::raw("'3' as type_id"))->get();
    //     $clarks = Clark::where('name','LIKE','%'.$searchkey.'%')->orwhere('clark_number','LIKE','%'.$searchkey.'%')->orwhere('motor_number','LIKE','%'.$searchkey.'%')->addSelect('*',DB::raw("'Clark' as type"),DB::raw("'4' as type_id"))->get();
    //     $equips = Equip::where('name','LIKE','%'.$searchkey.'%')->addSelect('*',DB::raw("'Equip' as type"),DB::raw("'5' as type_id"))->get();
    ...
    public function print_sanad_abd($type, $id)
    {
        ...
        if ($type == 'client') {
            $data = InvoiceClientMadyonea::where('id', $id)->with('client')->with('payment')->first();
            $paperTitle = " سند قبض ";
            $recordName = "العميل";
            // $recordName= $client->name;
        ...
        // return $presaleOrders;
        return view('sanad.print_sanad_abd', compact('payment_types', 'paperTitle', 'recordName', 'recordValue', 'recoredUrl', 'moneyVal', 'personName', 'datee'));
    }
    ...
    public function newClientQuick(Request $request)
    {
     ...
     public function talef($part_id,$source_id,$status_id,$quality_id,$type_id,$store_id){
        ...
        // return $amount;
        return view('talef',compact('stores','employee','item','source','status','quality','store_id','amount','type_id'));
    }
