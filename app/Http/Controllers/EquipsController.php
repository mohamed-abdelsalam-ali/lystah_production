...
class EquipsController extends Controller
{
    ...
    public function indexWithRequest(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('name_eng', function ($row) {
                    return isset($row->name_eng) ? $row->name_eng : '--';
                })
                ...
                ->addColumn('action', function ($row) {
                    ...
                    data-toggle="tooltip" data-original-title="Edit" class="btn btn-primary btn-sm d-inline-block editButton editEquipB "><i class="ri-edit-line editEquipButton"></i></a>';
                    $btn = $btn . ' <a  data-bs-toggle="modal"  equip_id_value2="' . $row->id . '"  data-bs-target="#deletepartB"
                    ...
                    // $btn = '<div class="row" style="width: fit-content;"><div class="px-1 mb-1 col"><a  data-bs-toggle="modal"  equip_id_value="'.$row->id.'"  data-bs-target="#editEquip"
                    //  data-toggle="tooltip" data-original-title="Edit" class="btn-sm editButton editEquipB"><i class="ri-edit-line editEquipButton"></i></a></div>';
                    //  $btn = $btn.' <div class="px-1 col"><form action="'.route("equip.destroy",$row->id).'" method="POST" class="form-block float-end">'
                    //          . csrf_field()
                    //          . method_field('DELETE')
                    ...
                    $btn = $btn . '<a href="' . route("equip.print", $row->id) . '"
                                class="   btn-info printButton  btn-sm d-inline-block printEquip mr-1"><i class=" ri-printer-fill"></i></a>';
    ...
    public function indexWithID(Equip $equipId)
    {
        ...
        $all_equip = AllEquip::where('part_id', $equipId->id)->with('sections')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            Equip::create([
                ...
                'last_sevice_date' => $request->last_sevice_date,
                'model_id' => $request->model_id,
                'supplayer_id' => $request->supplier_id,
                'company_id' => 10,
            ...
            if (isset($request->currency_id)  && isset($request->supplier_id)) {
                ...
                if (isset($total)) {
                    ...
                    if ($total >= ($request->invPaied * $all_currency_types[0]->currencies[0]->value)+$buyCoastAc) {
                        ...
                        $allpart = new AllEquip();
                        $allpart->part_id = $equip_id;
    ...
    public function update(Request $request, Equip $equip)
    {
        ...
        try {
            ...
            // }
            $equip->update([
                'name' => $request->name_edit, 'name_eng' => $request->name_eng_edit,
                'desc' => $request->desc_edit, 'year' => $request->year_edit,
                'hours' => $request->hours_edit, 'status_id' => $request->status_id_edit,
                'tank_capacity' => 0, 'limit_order' => 0,
                'flage_limit_order' => 0, 'last_sevice_date' => $request->last_sevice_date_edit,
                'model_id' => $request->model_id_edit, 'supplayer_id' => $request->supplier_id_edit,
                'company_id' => $request->company_id, 'currency_id' => $request->currency_id_edit,
                'source_id' => $request->source_id_edit, 'quality_id' => $request->quality_id_edit,
                'buy_price' => $request->price_edit
            ]);
            ...
            if ($bank) {
                ...
                BankSafeMoney::create([
                    'notes' => 'تعديل معدة ' . ' ' . $request->buyTransaction_id . ' ' . 'من مورد' . ' ' . $editSup->name,
                    'date' => date('Y-m-d'),
                    'flag' => 1,
            ...
            $StoreSection_deleted = StoreSection::where('order_supplier_id', $all_equip->order_supplier_id)->get();
            ...
            if (isset($request->currency_id_edit)  && isset($request->supplier_id_edit)) {
                ...
                if ($bank) {
                    ...
                    $total_sarf = MoneySafe::where('store_id', $store_bank->id)->where('type_money', 1)->sum('money'); //صرف
                    $total = floatval($raseed) +  floatval($total_qabd) - floatval($total_sarf)   +  floatval($xpaied * $xAc_all_currency_types[0]->currencies[0]->value);
                    // $total += floatval($xpaied * $xAc_all_currency_types[0]->currencies[0]->value);
                    $Ac_allTotal = MoneySafe::where('store_id', $store_bank->id)->latest()->first();
                }
                ...
                if (isset($total)) {
                    ...
                    if ($total >= $request->invPaied_edit * $all_currency_types[0]->currencies[0]->value) {
                        ...
                        DB::rollback();
                        session()->flash("success", "  المبلغ غير كافي في الخزنة" .  $total);
                        return redirect()->back();
                    }
                } else {
                    DB::rollback();
        ...
        // BuyTransaction::create(['date'=> Carbon::now(),'desc'=> 'Equip' , 'company_id'=> $request->company_id,'name' => Carbon::now(),  'final'=> '0' ,'creation_date'=> Carbon::now()]);
        ...
        // OrderSupplier::create(['user_id'=> Auth::user()->id ,'transaction_id' => $transaction_id , 'supplier_id' => $request->supplayer_id_edit,'status' =>'0' , 'container_size'=> $request->container_size_edit , 'total_price' => $request->price_edit ,'deliver_date'=> $request->deliverydate_edit , 'bank_account' => $request->bank_account_edit, 'currency_id' => $request->currency_id_edit ,'confirmation_date' => Carbon::now() ]);
        // $order_supplier_id = OrderSupplier::max('id');
        // Replyorder::create(['part_id' => $equip->id , 'amount' => '1' , 'order_supplier_id' => $order_supplier_id , 'creation_date' => Carbon::now(), 'part_type_id' =>'5' , 'price' => $request->price_edit, 'source_id' => $request->source_id_edit ,'quality_id' => $request->quality_id_edit, 'status_id' => $request->status_id_edit ]);
        // AllEquip::create(['part_id' => $equip->id , 'order_supplier_id' => $order_supplier_id , 'amount' => '1' , 'source_id' => $request->source_id_edit , 'status_id' => $request->status_id_edit , 'buy_price' => $request->price_edit , 'remain_amount' => '1' , 'flag' => '3' , 'quality_id' => $request->quality_id_edit , 'lastupdate' => Carbon::now()]);
