 ...
 * @package App\Models
 */
class AllWheel extends Model implements Auditable
{
        use \OwenIt\Auditing\Auditable;

    use \Awobaz\Compoships\Compoships;
	protected $table = 'all_wheels';
	...
	public function pricing(){
        return $this->hasMany(SalePricing::class, 'part_id','part_id' , 'source_id','source_id','status_id','status_id','quality_id','quality_id')->where('type_id',2)->where('to',null);
    ...
    {
        return $query->whereHas('wheel', function ($query) use ($value) {
            $query->where('name', 'like', "%{$value}%")->orWhere('name_eng', 'like', "%{$value}%")
                ->orWhereHas('wheel_numbers', function ($query) use ($value) {
    ...
    {
        return $this->hasOne(SalePricing::class, ['part_id', 'source_id', 'status_id', 'quality_id'], ['part_id', 'source_id', 'status_id', 'quality_id'])->where('type_id', 2)->where('to', null)
            ->orderByDesc('price'); // Order by price descending to get max price first
    }
    public function scopeBrand(Builder $query, $value)
    {
        return $query->whereRaw('1 = 0');
        // return $query->whereHas('wheel.wheel_model.series.model', function ($query) use ($value) {
    ...
    public function scopeModel(Builder $query, $value)
    {
        ...
        // return $query->whereHas('tractor.series', function ($query) use ($value) {
    ...
    public function scopeSeries(Builder $query, $value)
    {
        ...
        // return $query->whereHas('tractor', function ($query) use ($value) {
        //     $query->where('model_id', '=', $value);
        // });
    }
