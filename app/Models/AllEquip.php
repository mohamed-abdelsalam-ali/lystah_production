 ...
 * @package App\Models
 */
class AllEquip extends Model implements Auditable
{
        use \OwenIt\Auditing\Auditable;
        use \Awobaz\Compoships\Compoships;

	protected $table = 'all_equips';
    ...
    public function pricing(){
        return $this->hasMany(SalePricing::class, ['part_id','source_id','status_id','quality_id'], ['part_id','source_id','status_id','quality_id'])->where('type_id',5)->where('to',null);
    }
     ////////////////////////filters//////////////
    public function scopeName(Builder $query, $value)
    {
        return $query->whereHas('equip', function ($query) use ($value) {
            $query->where('name', 'like', "%{$value}%")->orWhere('name_eng', 'like', "%{$value}%");
        });
    }
    public function max_pricing()
    {
        return $this->hasOne(SalePricing::class, ['part_id', 'source_id', 'status_id', 'quality_id'], ['part_id', 'source_id', 'status_id', 'quality_id'])->where('type_id', 5)->where('to', null)
            ->orderByDesc('price'); // Order by price descending to get max price first
    }
    public function scopeBrand(Builder $query, $value)
    {
        return $query->whereHas('equip.series.model', function ($query) use ($value) {
            $query->where('brand_id', '=', $value);
        });
    }
    public function scopeModel(Builder $query, $value)
    {
        return $query->whereHas('equip.series', function ($query) use ($value) {
