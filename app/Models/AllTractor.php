 ...
 * @package App\Models
 */
class AllTractor extends Model  implements Auditable
{
        use \OwenIt\Auditing\Auditable;
        use \Awobaz\Compoships\Compoships;

	protected $table = 'all_tractors';
    ...
    public function sectionswithoutorder(){
        ...
        public function pricing(){
            return $this->hasMany(SalePricing::class, ['part_id','source_id','status_id','quality_id'], ['part_id','source_id','status_id','quality_id'])->where('type_id',3)->where('to',null);
        }
         ////////////////////////filters//////////////
    ...
    {
        return $this->hasOne(SalePricing::class, ['part_id', 'source_id', 'status_id', 'quality_id'], ['part_id', 'source_id', 'status_id', 'quality_id'])->where('type_id', 3)->where('to', null)
            ->orderByDesc('price'); // Order by price descending to get max price first
    }
    public function scopeBrand(Builder $query, $value)
    {
        return $query->whereHas('tractor.series.model', function ($query) use ($value) {
