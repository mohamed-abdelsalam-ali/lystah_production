...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            $("#dimtypeSlct").empty();
            $("#dimtypeSlct").append(`<option disabled selected value="0">Select Dimensions</option>`);
    ...
    $.ajax({
        ...
        success: function (response) {
            $("#modeltypeSlct").empty();
            $("#modeltypeEditSlct").append(`<option disabled selected value="0">Select Brand</option>`);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#materialtypeSlct").append(`<option disabled selected value="0">Select Material</option>`);
            $("#materialtypeEditSlct").empty();
            $("#materialtypeEditSlct").append(`<option disabled selected value="0">Select Material</option>`);
            response.forEach(element => {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#relatedWheelSlct").append(`<option disabled value="0">Select Wheels</option>`);
            $("#relatedWheelEditSlct").empty();
            $("#relatedWheelEditSlct").append(`<option disabled value="0">Select Wheels</option>`);
            response.forEach(element => {
...
$(document).on('click', ".editwheelB" , function (e) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#div4").hide();
            $("#typeEditSlct").empty();
            $("#relatedWheelEditSlct").append(`<option disabled selected value="0">Select Wheels</option>`);
            $("#wheelِEditTbl").empty();
            $("#relatedwheelِEditTbl tbody").empty();
            $("#wheelSpecsEditTbl").empty();
            $(".upload__img-wrapWheelEdit").empty();

            if(wheelType[0] =='كاوتش'){
                $("#typeEditSlct").append(`
                ...
                $("#typeEditSlct").append(`
                <option value="0" selected>اختر نوع العجلة</option>
                <option value="1">كاوتش</option>
                <option value="2">جنط</option>
                <option value="3">داخلي</option>
                `);
            ...
            if(wheel.wheel_dimension != null ){
                ...
                $("#dimtypeEditSlct").append(`<option disabled selected value="0">Select Dimensions</option>`);
            }
            ...
            if(wheel.wheel_model != null ){
                ...
                $("#modeltypeEditSlct").append(`<option disabled selected value="0">Select Model</option>`);
            }
...
$(document).on('change', '#typeSlct', function(){
    ...
    $("#name").val(textVal+"/"+dimText+"/"+modelText+"/"+materialText+"/"+radioVal);
...
$(document).on('change', '#typeEditSlct', function(){
    ...
    $("#nameEdit").val(textVal+"/"+dimText+"/"+modelText+"/"+materialText+"/"+radioVal);
    $("#dimtypeEditSlct").append(`<option disabled selected value="0">Select Dimension</option>`);
    $("#modeltypeEditSlct").append(`<option disabled selected value="0">Select Brand</option>`);
    $("#materialtypeEditSlct").append(`<option disabled selected value="0">Select Material</option>`);
});
...
$(document).on('change', '#dimtypeEditSlct', function(){
    $("#nameEdit").val($("#typeEditSlct option:selected").text()+"/"+$("#dimtypeEditSlct option:selected").text()+"/"+$("#modeltypeEditSlct option:selected").text()+"/"+$("#materialtypeEditSlct option:selected").text()+"/"+$('input[name="tt_tlEdit"]:checked').attr('data-nameEdit'));
...
$(document).on('change', '#relatedWheelSlct', function(){
    ...
    if($.inArray(wheelId,wheelArr) >= 0){
        ...
        $("#wheelTbl").append(`<tr>
            ...
            <td onclick="$(this).closest('tr').remove();wheelArr.splice(wheelArr.indexOf('${wheelId}'), 1);">Remove</td>
        </tr>`)
    }
 });
