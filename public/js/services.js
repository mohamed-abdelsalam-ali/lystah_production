...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
          $("#name").empty();
          $("#name").append(`<option  disabled selected value="0">اختر إسم العميل</option>`);
          response.forEach(element =>{
...
$("input:radio[name='serviceOption']").change(function() {
  ...
  if($("input[name='serviceOption']:checked").val() == "internal"){
    ...
    $("input:radio[name='serviceType']").each(function(i) {
      this.checked = false;
    });
  ...
  if($("input[name='serviceType']:checked").val() == "Tractor"){
    ...
    $.ajax({
      ...
      success: function (response) {
          ...
          if (selecturl == 'clark') {
            response.forEach(element =>{
              $("#optionSelect").append(`<option value="${element.stores_log.all_clarks[0].clark.id}">${element.stores_log.all_clarks[0].clark.name} / ${element.stores_log.all_clarks[0].clark.clark_number}</option>`);
            });
          }else if(selecturl == 'tractor') {
            response.forEach(element =>{
              $("#optionSelect").append(`<option value="${element.stores_log.all_tractors[0].tractor.id}">${element.stores_log.all_tractors[0].tractor.name} / ${element.stores_log.all_tractors[0].tractor.tractor_number}</option>`);
            });
...
$(document).on('keyup', '#totalPaid', function(e) {
  ...
  $('#remain').val($('#paymentFinalValue').text() - $('#totalPaid').val()); 
});
...
function calcTotal(){
  ...
  $("#addServicesTbl .servicePrice").each(function() {
    ...
    $("#remainValue").text(totalPrice - $('#totalPaid').val());
...
function calcTaxes(){
  ...
  $('#serviceTaxTbl > tr').each(function() {
    ...
    $("#remainValue").text($("#paymentFinalValue").text() - $('#totalPaid').val());
...
function calcDiscount(){
  ...
  $('#remainValue').text(parseFloat($('#paymentFinalValue').text()) - parseFloat($('#totalPaid').val())); 
